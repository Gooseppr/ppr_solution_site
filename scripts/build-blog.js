...
function renderPostCard(post, featured = false) {
  ...
  return `<article class="${className}" ${postDataAttributes(post)}><a class="article-card-link" href="blog/${escapeHtml(post.slug)}/">${label}<p class="meta">${escapeHtml(post.category || "Blog")} · <time datetime="${escapeHtml(post.date)}">${formatDateFr(post.date)}</time> · ${escapeHtml(post.reading_time || "Lecture")}</p><${headingTag}>${escapeHtml(post.title)}</${headingTag}><p>${escapeHtml(post.description)}</p>${renderPostPreview(post.preview)}${renderTags(post.tags)}<span class="blog-read-label">Lire l’article →</span></a></article>`;
}
...
function renderBlog(posts) {
  ...
  const cards = sorted.slice(1).map((post) => renderPostCard(post)).join("");
  ...
  return `<!DOCTYPE html><html lang="fr"><head>${head({ title: "Ressources documentation, données et technologies · PPR-Solution", description: "Méthodes et analyses autour de la documentation structurée, des formats techniques, de l’automatisation et de l’IA appliquée.", canonical: `${BASE_URL}/blog.html`, prefix: "", type: "website" })}<script type="application/ld+json">${safeSchema}</script></head><body><a class="skip-link" href="#main-content">Aller au contenu</a>${nav("")}<main id="main-content"><section class="section-tight blog-library-section" id="blog-library" aria-labelledby="blog-library-title"><div class="container"><header class="blog-library-intro"><div><p class="eyebrow">Ressources &amp; veille</p><h1 id="blog-library-title">Documentation, données et technologies</h1></div><p class="lead">Méthodes, analyses et actualités autour de la documentation structurée, des formats techniques, de l’automatisation et de l’IA.</p></header><form class="blog-controls" data-blog-controls hidden><div class="blog-control-primary"><div class="blog-control-field blog-search-field"><label for="blog-search">Rechercher</label><input id="blog-search" type="search" placeholder="Documentation, XML, automatisation…" autocomplete="off" data-blog-search></div><div class="blog-control-field blog-sort-field"><label for="blog-sort">Trier</label><select id="blog-sort" data-blog-sort><option value="newest">Plus récent</option><option value="oldest">Plus ancien</option><option value="title-asc">Titre A–Z</option><option value="title-desc">Titre Z–A</option></select></div></div><details class="blog-filter-details"><summary>Filtres <span>Catégorie et sujet</span></summary><div class="blog-filter-grid">${categoryControl}${tagControl}</div></details><div class="blog-control-status"><p id="blog-result-count" role="status" aria-live="polite"><strong>${posts.length}</strong> articles</p><button class="btn btn-text" type="button" data-blog-reset hidden>Réinitialiser</button></div></form><p class="blog-data-status" data-blog-data-status role="status" hidden></p><div class="blog-empty-state" data-blog-empty role="status" hidden><p class="eyebrow">Aucun résultat</p><h2>Aucun article ne correspond à cette recherche.</h2><p>Modifiez les critères ou affichez toute la bibliothèque.</p><button class="btn" type="button" data-blog-empty-reset>Afficher tous les articles</button></div><div data-blog-results><div data-blog-featured>${renderPostCard(featured, true)}</div><div class="blog-card-grid" data-blog-grid>${cards}</div><nav class="blog-pagination" aria-label="Pagination des articles" data-blog-pagination hidden></nav></div></div></section></main>${footer("")}<script src="assets/js/main.js" defer></script><script src="assets/js/blog.js" defer></script></body></html>`;
}
